...
For RAG context generation (feeding the parser grimoire history during Consistency scans), AllKnower sends the fused RRF candidate list to OpenRouter's native `/rerank` endpoint using `cohere/rerank-4-pro`. 

The reranker scores how closely the contents of the top 20 notes answer the specific AI prompt. It returns the top 5 most relevant documents, which AllKnower injects into the parser context window. This saves token costs and prevents parser "lost in the middle" retrieval failures.
